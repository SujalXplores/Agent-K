...
const chartValues = [
	14, 12, 15, 13, 17, 15, 19, 18, 24, 22, 30, 44, 69, 81, 76, 88, 84, 93,
] as const;
const stageIcons = [
	TriangleAlert,
	Search,
...
export function IncidentSimulation() {
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener(INCIDENT_REPLAY_EVENT, handleReplay);
	}, [runSimulation]);
	...
	const gateState =
		activeStage >= 4 ? "verified" : activeStage >= 3 ? "allowed" : "idle";
	const gateChecks =
		activeStage >= 3 ? 6 : activeStage >= 2 ? 3 : activeStage >= 1 ? 1 : 0;
	const buttonLabel = running ? "Restart" : activeStage >= 4 ? "Replay" : "Run";
	const stageProgress =
		activeStage < 0 ? 0 : (activeStage + 1) / incidentStages.length;
	...
	return (
		...
		>
			<ShineBorder tone="signal" />
			<header className={styles.consoleHeader}>
				<div>
					<span className={styles.consoleK} aria-hidden="true">
						K
					</span>
					<span>
				...
				<span
					className={`${styles.recording} ${running ? styles.recordingActive : ""}`}
				>
					<i aria-hidden="true" />
					Controlled trace
				</span>
			...
			>
				...
				<div className={styles.chartField} aria-hidden="true">
					...
					<div className={styles.deployMarker}>
						<span>deploy v2</span>
					</div>
				...
				<div className={styles.stageColumn}>
					...
					<ol className={styles.stages} aria-label="Investigation stages">
						{incidentStages.map((stage, index) => {
							...
							const isCurrent =
								activeStage === index && index !== incidentStages.length - 1;
							const isComplete =
								activeStage > index || (activeStage === 4 && index === 4);
							return (
								<m.li
									...
									initial={false}
									animate={{
										opacity: isCurrent || isComplete ? 1 : 0.38,
										x: isCurrent ? 6 : 0,
									}}
									transition={
										reduceMotion
					...
					<ul>
						{incidentEvidence.map((item, index) => {
							...
							return (
								...
								>
									<span
										className={`${styles.evidenceIcon} ${styles[item.kind]}`}
									>
			...
			<footer className={`${styles.gate} ${styles[gateState]}`}>
				...
				<span className={styles.gateCopy}>
					...
					<span
						className={styles.statusViewport}
						role="status"
						aria-live="polite"
						aria-atomic="true"
					>
					...
					<span className={styles.gateChecks} aria-hidden="true">
						{Array.from({ length: 6 }).map((_, i) => (
							<m.i
								...
								animate={{
									backgroundColor:
										i < gateChecks ? "var(--acid)" : "rgba(255,255,255,0.08)",
									scaleX: i < gateChecks ? 1 : 0.4,
								}}
								transition={
									reduceMotion
										? MOTION_TRANSITION.instant
										: { ...MOTION_TRANSITION.fast, delay: i * 0.04 }
								}
							/>
						))}
			...
			<p className={styles.disclaimer}>
				Controlled scenario · no production action
			</p>
		</article>
	);
}
