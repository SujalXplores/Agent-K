...
- [ ] Rollback executor: Agent K has direct access to the monitored app's docker-compose.yml/.env on the same host — edits the image tag, runs `docker compose up -d`, creates a SigNoz deployment marker, waits, re-queries SigNoz to verify recovery, records the outcome
...
| Rollback executor has direct docker-compose access on host | Simplest sandboxed-rollback implementation achievable in the timeline; "sandbox" = scoped to this one allowlisted action, not a separate service | — Pending |
