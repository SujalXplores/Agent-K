...
- [ ] **Phase 6: Policy Gate + Rollback Executor** - Build Law 2's zero-LLM policy gate and the single allowlisted, verified-outcome rollback action
...
**Goal**: Every action Agent K could take passes through a deterministic, zero-LLM policy gate, and the one allowlisted rollback action executes safely and only when policy allows it, with the outcome independently re-verified against SigNoz.
**Depends on**: Phase 5
...
**Success Criteria** (what must be TRUE):
  ...
  4. An approved rollback captures the pre-mutation image tag, edits the monitored app's `docker-compose.yml`/`.env`, runs `docker compose up -d` (never `restart`), creates a SigNoz deployment marker, and re-queries SigNoz to confirm recovery before recording the verified outcome.
...
**Plans**: TBD
**Constraint**: Safety-critical phase (rollback executor mutates the monitored app's running config) landing inside or just before the team-availability gap (Jul 24-26) — the docker-socket-vs-host-process decision and policy schema should be settled early, not debugged with reduced headcount.
