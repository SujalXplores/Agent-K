...
**Domain:** AI-native FastAPI RAG service + code-enforced incident-response agent (Agent K), observed via SigNoz/OpenTelemetry
**Researched:** 2026-07-20
**Confidence:** MEDIUM (package versions verified live against PyPI = HIGH; ecosystem/behavioral claims from web search only = LOW; flagged per-row below)
...
pip install "fastapi==0.139.2" "uvicorn[standard]==0.51.0" \
  "sqlalchemy==2.0.51" "asyncpg==0.31.0" "pgvector==0.5.0" "alembic==1.18.5" \
  "pydantic==2.13.4" "python-dotenv"
...
pip install "openai==2.46.0" "sentence-transformers==5.6.0"
# optional:
pip install "litellm==1.93.0"

# Agent K process (separate from the RAG app; can share a venv for hackathon speed)
pip install "mcp==1.28.1" "openai==2.46.0" "pydantic==2.13.4" "httpx"
...
**If the team is behind schedule by Day 3-4 on SigNoz/Foundry setup:**
...
**If retrieval-quality demos look weak with `all-MiniLM-L6-v2`:**
...
**If GenAI semconv attribute names cause confusion mid-build:**
...
---
*Stack research for: AI-native FastAPI RAG service + SigNoz-integrated incident-response agent (Agent K)*
*Researched: 2026-07-20*
