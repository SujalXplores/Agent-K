...
last_activity: 2026-07-25 — Added demo/ (Flowdeck Support, cloned from assistant-ui) over the untouched POST /ask; recorded the hosted-demo topology in HOSTED-DEMO.md. Hosted SigNoz is blocked on human account creation.
progress:
  total_phases: 7
  completed_phases: 2
  total_plans: 15
...
- [Demo surface, 2026-07-25]: Added `demo/` — "Flowdeck Support", a help-centre chat UI cloned and owned from assistant-ui (MIT, `assistant-ui create -t minimal`), branded to the fictional SaaS the seeded 72-doc corpus is already written about. **Scope-lock note:** this is a presentation surface, not a locked-scope feature — it adds no requirement to REQUIREMENTS.md and removes nothing. It exists because the only prior way to show a seeded failure was curling a JSON endpoint. The monitored FastAPI service is untouched: the LLM call stays inside FastAPI (or every `gen_ai.*` span, failure flag and eval run would be bypassed), and the browser never reaches FastAPI (no CORS added — `demo/app/api/ask` proxies server-side). Removed `@ai-sdk/openai`, `ai`, `@assistant-ui/react-ai-sdk` and the `app/api/chat` route from the upstream scaffold; the app holds no model API key. See `demo/README.md`.
- [Demo surface, 2026-07-25]: assistant-ui's `useAuiState` selections must be referentially stable. `s.message.parts` is rebuilt on every snapshot read, so selecting it — or anything derived from it via `.filter`/`.map` — throws "getSnapshot should be cached to avoid an infinite loop" and drives a render loop that aborts in-flight fetches. Fixed in `demo/components/assistant-ui/thread.tsx` by selecting a JSON *string* fingerprint (strings compare by value) and decoding it for render. Any future `useAuiState` selector in this app must return a primitive or a stable reference.
- [Demo console, 2026-07-25]: `demo/console` runs each seeded incident in one click via the same sequence as `scripts/run_eval.py` (set flag → traffic → alert webhook), and renders each Law separately. Two contract details were found only by running it live: the alert envelope requires top-level `receiver` and `status` (omitting them is a 422, not a lenient parse), and the whole-second `Z` timestamp format is load-bearing because the investigation freezes its evidence window from `startsAt`. Traffic generation must swallow `/ask` failures — under db_pool_exhaustion those requests are meant to fail and that failure is the signal.
- [Demo fixture, 2026-07-25]: `scripts/demo_mcp_server.py` is a real MCP stdio server standing in for SigNoz, selected via `SIGNOZ_MCP_COMMAND`, so every capability is demonstrable offline and deterministically. **Never the default** — compose passes the var through empty so an unconfigured stack falls back to the real binary and fails loudly. Kept honest three ways: `_source: "agent-k demo fixture (NOT live SigNoz)"` on every payload; it reads live flag state from `/admin/flags` so it cannot be told what to conclude; and it emits no deployment marker for the two runtime faults, preserving absence-as-evidence. **Its evidence links do not resolve** — Law 1's resolve claim still needs a real instance. Also added `SIGNOZ_MCP_ARGS` (StdioServerParameters was built without argv) and made empty-string env count as unset, because compose's `${VAR:-}` sets `""` rather than leaving a var unset.
- [Law 3 guardrails, 2026-07-25]: `LOOP_BREAKER_REPEAT_THRESHOLD`/`TOKEN_BUDGET` became `loop_breaker_threshold()`/`token_budget()`, reading env at call time so a demo can tighten a threshold and watch the real guardrail fire on the real code path. A malformed value falls back to the default rather than coercing to 0/infinity — asserted directly, because a typo in a demo env var must not be able to disable a safety guardrail.
- [Environment, 2026-07-25]: This Windows machine has **only Python 3.14.3**, which the project's own pins reject (3.11/3.12; torch/sentence-transformers wheels lag). The containerized path is therefore the supported one here — `Dockerfile` is `python:3.12-slim` with the model baked in. Tests run via `docker run` with `app/`, `tests/`, `deployer/` mounted over the baked copies. Also: host port 5432 is held by an unrelated `synex_postgres` container, remapped to 5433 in a gitignored `docker-compose.override.yaml` (needs `ports: !override` — Compose appends list fields).
- [Hosted demo, 2026-07-25]: Judge-facing dashboard hosts on **SigNoz Cloud** (30-day trial, no credit card) rather than a public VM — chosen for speed, supports public dashboard sharing, and the MCP server works against Cloud unchanged. Requires no code change: `app/telemetry.py` passes `endpoint=` explicitly but leaves `headers` unset, so the SDK's `OTEL_EXPORTER_OTLP_HEADERS` env fallback carries the ingestion key. The Foundry `casting.yaml`/`.lock` path in SIGNOZ-RUNBOOK.md is unchanged and remains the reproducible install story. Two limits recorded honestly in HOSTED-DEMO.md: the rollback needs the Docker socket (the deployer sidecar's structural isolation boundary), which no free PaaS grants, so Law 2 only genuinely executes on a controlled host; and evidence deep links require a login on Cloud *and* self-hosted alike, so the Law 1 claim is "100% resolve, verified by the automated checker", not "any reader can click through".
...
- [Demo, BLOCKING, human-only]: Put a real `GROQ_API_KEY` in the repo-root `.env`. **This is the only thing standing between the current stack and a fully watchable demo.** Everything else is verified working against the live stack: Postgres + 72 seeded docs, exact retrieval, the fixture MCP answering Agent K's real `query_signoz()`, the alert webhook creating an investigation, and the console rendering it. With no key, `/ask` serves nothing and every investigation escalates with zero claims — which is correct fail-fast behaviour (CR-04) but unwatchable.
- [Hosted demo, BLOCKING, human-only]: Create the SigNoz Cloud account (30-day trial, no card) and record region / ingestion key / API key. **An agent cannot do this — account creation is a human action.** Everything downstream of a hosted dashboard is blocked on it: importing `dashboards/agent-k.json`, publishing it, and closing EVAL-04's screenshots/footage. Steps in HOSTED-DEMO.md.
- [Hosted demo]: Once SigNoz Cloud is live, run the app against it and confirm spans actually arrive before trusting the dashboard — a wrong ingestion key fails silently. Then import the dashboard *after* traffic exists, since empty widgets mean "no data yet", not "broken query".
- [Hosted demo]: `demo/` has not yet been exercised against the real RAG service — only against a throwaway stand-in that mimics the `{answer, sources}` and `{flags}` contracts. Confirm the real `/ask` and `/admin/flags` drive it identically.
